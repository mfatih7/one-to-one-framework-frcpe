...
def plot_mAP_exact( config, epoch, chunk, mAP_exact_checkpoint, ref_angles,):
    ...
    if(epoch>0 or chunk>0):
        ...
        if(mAP_exact_checkpoint.shape[0]==2):
            sets = ['train', 'val']
            linestyles = ['-', ':']
        else:
            sets = ['test']
            linestyles = ['--']
        x_count = mAP_exact_checkpoint.shape[1] * mAP_exact_checkpoint.shape[2]
        
        error_angle_legend_names = ['R', 't', 'Rt']
        
        for ref_angle_ind, ref_angle in enumerate(ref_angles):
        
            mAP = np.zeros( (len(sets), mAP_exact_checkpoint.shape[3], x_count) )
            
            for set_ind, set_ in enumerate(sets):
                for err_ang in range(mAP_exact_checkpoint.shape[3]):
                    for e in range(mAP_exact_checkpoint.shape[1]):
                        for c in range(mAP_exact_checkpoint.shape[2]):
                            mAP[set_ind, err_ang, e*mAP_exact_checkpoint.shape[2]+c] = mAP_exact_checkpoint[set_ind, e, c, err_ang, ref_angle_ind]
            mAP = mAP[:,:,0:epoch*config.n_chunks+chunk+1]
            x_line = np.arange(0, epoch*config.n_chunks+chunk+1, 1)
            ...
            plt.figure()
            plt.xlabel('Chunk')
            plt.title( 'mAP_exact_' + f'{ref_angle:02d}' + '_degree' )
            ...
            plt.legend(fontsize = 'x-small', loc="lower right", ncol=3,)
            if(mAP.shape[0]==2):
                plot_file_name = 'mAP_exact_' + f'{ref_angle:02d}' + '_degree.png'
            else:
                plot_file_name = 'mAP_exact_test_' + f'{ref_angle:02d}' + '_degree.png'
            plt.savefig( os.path.join(folder_name, plot_file_name), dpi=300)        
