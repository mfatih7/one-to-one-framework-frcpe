...
### Generating dataset, sampler and dataloader for the current test chunk
            ...
            with torch.no_grad():        
                for i, data in enumerate(dataloader_test):
                    ...
                    confusion_matrix_at_epoch_test_device[0,0] += torch.sum( torch.logical_and( logits[-1][-1]<0, ys_ds[-1][-1]>config.obj_geod_th ) )
                    confusion_matrix_at_epoch_test_device[0,1] += torch.sum( torch.logical_and( logits[-1][-1]>0, ys_ds[-1][-1]>config.obj_geod_th ) )
                    confusion_matrix_at_epoch_test_device[1,0] += torch.sum( torch.logical_and( logits[-1][-1]<0, ys_ds[-1][-1]<config.obj_geod_th ) )
                    confusion_matrix_at_epoch_test_device[1,1] += torch.sum( torch.logical_and( logits[-1][-1]>0, ys_ds[-1][-1]<config.obj_geod_th ) )
...
###############################################################################################################   
                    ...
                    for b in range( xs_ess.shape[0]):
                        ...
                        R_b = data['R'][b]
                        t_b = data['t'][b]
                        E_hat_b = e_hat[0][b].unsqueeze(dim=0)
                        y_hat_b = logits[-1].detach()[b].unsqueeze(dim=-1)
                        
                        xs_ds_b = xs_ds[-1][b].cpu()
                    
                        # err_q, err_t, err_qt = mAP.calculate_err_q_err_t( config=config, xs_ess=xs_ess_b, R=R_b, t=t_b, E_hat=E_hat_b, y_hat=y_hat_b )
                        err_q, err_t, err_qt = mAP.calculate_err_q_err_t( config=config, xs_ess=xs_ds_b, R=R_b, t=t_b, E_hat=E_hat_b, y_hat=y_hat_b )
    ...
    mAP_exact_checkpoint = xm.all_reduce( reduce_type = xm.REDUCE_SUM, inputs = torch.tensor( mAP_exact_checkpoint, dtype=torch.float64).to(device) )
    ...
    mAP_exact_checkpoint = mAP_exact_checkpoint / world_size
    ...
    mAP_exact_checkpoint = mAP_exact_checkpoint.detach().cpu().numpy()
