...
from loss_module.loss_CLNET import MatchLoss
import checkpoint
import plots
...
### Generating dataset, sampler and dataloader for the current train chunk
            ...
            for i, data in enumerate(dataloader_train):
                ...
                logits, xs_ds, ys_ds, e_hat, y_hat = model( xs_device, labels_device.squeeze(dim=-1) )
                
                classif_loss, ess_loss, geo_loss = match_loss.run( R_device, t_device,  xs_device, virtPt_device, logits, ys_ds, e_hat, y_hat)
                ...
                confusion_matrix_at_epoch_train_device[0,0] += torch.sum( torch.logical_and( logits[-1][-1]<0, ys_ds[-1][-1]>config.obj_geod_th ) )
                confusion_matrix_at_epoch_train_device[0,1] += torch.sum( torch.logical_and( logits[-1][-1]>0, ys_ds[-1][-1]>config.obj_geod_th ) )
                confusion_matrix_at_epoch_train_device[1,0] += torch.sum( torch.logical_and( logits[-1][-1]<0, ys_ds[-1][-1]<config.obj_geod_th ) )
                confusion_matrix_at_epoch_train_device[1,1] += torch.sum( torch.logical_and( logits[-1][-1]>0, ys_ds[-1][-1]<config.obj_geod_th ) )
            ...
            if(config.validation == 'enable'):
                ...
                with torch.no_grad():        
                    for i, data in enumerate(dataloader_val):
                        ...
                        confusion_matrix_at_epoch_val_device[0,0] += torch.sum( torch.logical_and( logits[-1][-1]<0, ys_ds[-1][-1]>config.obj_geod_th ) )
                        confusion_matrix_at_epoch_val_device[0,1] += torch.sum( torch.logical_and( logits[-1][-1]>0, ys_ds[-1][-1]>config.obj_geod_th ) )
                        confusion_matrix_at_epoch_val_device[1,0] += torch.sum( torch.logical_and( logits[-1][-1]<0, ys_ds[-1][-1]<config.obj_geod_th ) )
                        confusion_matrix_at_epoch_val_device[1,1] += torch.sum( torch.logical_and( logits[-1][-1]>0, ys_ds[-1][-1]<config.obj_geod_th ) )
