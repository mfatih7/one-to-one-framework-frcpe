...
### Generating dataset, sampler and dataloader for the current train chunk
            ...
            for i, data in enumerate(dataloader_train):
                ...
                for j in range( len(logits) ):
                    ...
                    classif_loss = loss_functions_n_to_n.get_losses( config, device, labels_device, logits[j] )
                                
                    geo_loss, ess_loss, _ = loss_functions_n_to_n.calculate_ess_loss_and_L2loss( config, logits[j], xs_ess, R_device, t_device, virtPt_device )
                ...
                confusion_matrix_at_epoch_train_device[0,0] += torch.sum( torch.logical_and( logits[-1]<0, labels_device.squeeze()>config.obj_geod_th ) )
                confusion_matrix_at_epoch_train_device[0,1] += torch.sum( torch.logical_and( logits[-1]>0, labels_device.squeeze()>config.obj_geod_th ) )
                confusion_matrix_at_epoch_train_device[1,0] += torch.sum( torch.logical_and( logits[-1]<0, labels_device.squeeze()<config.obj_geod_th ) )
                confusion_matrix_at_epoch_train_device[1,1] += torch.sum( torch.logical_and( logits[-1]>0, labels_device.squeeze()<config.obj_geod_th ) )
                ...
                if( ( (i*batch_size) % 10000 ) > ( ((i+1)*batch_size) % 10000 ) or (i+1) == len(dataloader_train) ):
            ...
            if(config.validation == 'enable'):
                ...
                with torch.no_grad():        
                    for i, data in enumerate(dataloader_val):
                        ...
                        geo_loss, ess_loss, _ = loss_functions_n_to_n.calculate_ess_loss_and_L2loss( config, logits[-1], xs_ess, R_device, t_device, virtPt_device )
    
                        confusion_matrix_at_epoch_val_device[0,0] += torch.sum( torch.logical_and( logits[-1]<0, labels_device.squeeze()>config.obj_geod_th ) )
                        confusion_matrix_at_epoch_val_device[0,1] += torch.sum( torch.logical_and( logits[-1]>0, labels_device.squeeze()>config.obj_geod_th ) )
                        confusion_matrix_at_epoch_val_device[1,0] += torch.sum( torch.logical_and( logits[-1]<0, labels_device.squeeze()<config.obj_geod_th ) )
                        confusion_matrix_at_epoch_val_device[1,1] += torch.sum( torch.logical_and( logits[-1]>0, labels_device.squeeze()<config.obj_geod_th ) )
                        ...
                        if( ( (i*batch_size) % 10000 ) > ( ((i+1)*batch_size) % 10000 ) or (i+1) == len(dataloader_val) ):
