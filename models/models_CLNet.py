...
class CLNet(nn.Module):
    def __init__(self, ):
        ...
        self.ds_0 = DS_Block(initial=True, predict=False, out_channel=128, k_num=9, sampling_rate = self.sampling_rate )#1.0)
        self.ds_1 = DS_Block(initial=False, predict=True, out_channel=128, k_num=6, sampling_rate = self.sampling_rate )
    ...
    def forward(self, x, y):
        ...
        return ws0 + ws1, [x, x, x1, x1, x2], [y, y, y1, y1, y2], [e_hat], y_hat
    
    
def get_model_CLNet( config, N, model_width, en_checkpointing ):
